...
图文展示的是"长什么样"，这里回答"**开了到底有什么用**"。仓库内建一套可复现的真机对比基准（[`bench/`](./bench/)）：同一工作流在**新会话**中执行两遍，对比开记忆与关记忆的第二遍表现。记忆的价值不在第一遍——开不开都要试错；而在第二遍：开记忆的 Agent 召回上次上下文，不再询问登录信息、不再重走死路，直取已验证路径；关记忆的每次都从零试错，不长记性。

每个测试集按固定矩阵执行 4 次（顺序 A1→A2→B1→B2，每次全新会话）：

| 执行 | 记忆 | 度量意义 |
|---|---|---|
| A1 / B1 | 开 / 关 | 首遍基线，两者应大致相当（公平性自检） |
| **A2 vs B2** | 开 vs 关 | **核心对比**：第二遍执行，记忆收益所在 |

三个测试集各押一种记忆类型——**凭据与流程**（登录 B 站 → 热门榜 → 下载视频）、**用户偏好**（"按老规矩"整理下载目录，含命名格式细节）、**可行路径**（pip 装包走镜像，看第二遍首条命令是否自带 `-i`）。指标六项：输入/输出 token、步骤数、墙钟耗时、反问用户次数、失败尝试次数、任务完成度，手动录入 [`bench/results.md`](./bench/results.md)。

> **首批实测数据录制中，完成后在此发布**（含执行环境声明）。

方法论要点与局限（与发布数字同源声明）：单人单机、每格单次执行、结果随模型与网络波动（非确定性）；固定执行顺序使练习效应对记忆收益为**保守方向**；剧本作者即插件作者，任务选取偏向记忆优势场景——欢迎按 [bench/playbook.md](./bench/playbook.md) 自行复现检验。
