...
三个测试集各押一种记忆类型——**凭据与流程**（登录 B 站 → 热门榜 → 下载视频）、**用户偏好**（"按老规矩"整理下载目录，中途改命名规矩）、**可行路径**（pip 装包走镜像，看第二遍首条命令是否自带 `-i`）。效率六项指标 + 探针准确率手动录入 [`bench/results.md`](./bench/results.md)，逐步操作手册见 [`bench/runbook.md`](./bench/runbook.md)；记忆系统自身的蒸馏开销作为可选成本行如实计入（只报节省不报成本是耍赖）。
