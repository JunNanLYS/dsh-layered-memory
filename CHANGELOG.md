...
- **宿主支持扩展到 0.1.2-rc.1**：对齐 0.1.2-rc.1 宿主组件；会话事件读取迁移到
  `snapshotEvents()`（rc.1 移除 `events` getter，旧宿主自动回退）。npm latest 已指向
  0.1.2-rc.1，安装命令不再需要钉版本号。
- **依赖管理由 npm 迁移到 pnpm**：lockfile 换为 `pnpm-lock.yaml`，CI 与发布流水线安装
  步骤改走 pnpm（发布仍上 npm registry，本地嵌入运行时安装保持 npm——Node 自带、
  用户机零前提）。配置 schema 增加显式类型注解。
