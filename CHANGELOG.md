...
> 0.9.1 因截图含用户隐私已从 npm 撤回，0.9.2 为同内容干净版（无截图）。

### 修复（会话面级联菜单交互）

- **数据流 hover 子卡片视口适配**：子卡片默认自「数据流」行右下浮出，行下方空间
  （`innerHeight` − 8px 边距）放不下整卡时自动**上翻**为底边锚定行底向上生长
  （宿主 tooltip 浮层同款判定式），两侧都不足时按可用侧夹持 `max-height` 滚动，
  窗口滚动/缩放实时重算。输入栏贴视口底的结构下行下方恒约 120px，上翻即常态
  路径——修复子卡片矩形底部超出窗口可视范围的问题。
- **浮层 overflow 纪律**（两轮回归的根因收口）：级联菜单与子卡片仅在限高真正收紧
  （矮窗口）时才开 `overflow-y: auto`——子卡片是菜单内绝对定位元素，`::before`
  桥接热区在卡片盒外左侧，常态开滚动会把二者整体裁掉（表现为子卡片不显示、
  或鼠标穿隙时 hover 链断裂卡片收起）。
- **菜单整体限高**：接入宿主 `useAnchoredMaxHeight` 原语（缺失时本地同式回退，
  `rect.bottom − 12`），矮窗口下菜单自身不再捅出视口顶。

## [0.9.0] — 2026-08-31

### 变更（UI 全面重构：分散式会话面 + 记忆工作台）

大范围 UI 重构（spec 驱动 + 可交互样例批准门禁）：会话侧信息不再集中在一块插件自有
条带，而是**按信息类型分散进宿主原生座位**；设置侧从六 Tab 浏览器重写为**五区任务
工作台**。全程走「决策 → spec → 非生产样例 → 用户批准 → 生产实施」流程。

#### 会话侧（分散式）

- **记忆芯片**取代档位 pill：输入栏左簇、Read Only 芯片右侧的 `记忆 · {智能|日常|工作}`
  无边框芯片，数值逐项对齐原生访问模式触发钮（真机实测：字重 500 / 圆角 24 / padding
  8·4 / chevron 为原生 14×14 SVG 逐字复刻，图标色双主题同值），文案 = 解析真值
  （只写/暂停/降级态带语义点），全部走 zh/en 字典（随宿主语言切换）。
  级联菜单行尺寸同步原生菜单条目（14px / 40px 高 / padding 8·10 / 圆角 10，浮层间距
  4px、min-width 218）。
  ![记忆芯片与级联菜单](./assets/changelog/0.9.0/01-记忆芯片级联菜单.png)
- **级联菜单**：芯片点击向上展开两行（记忆范围/数据流 + 行尾当前值）；数据流选项在
  hover 二级子面板（跟随全局/读写/只写/暂停），点击不固定、`::before` 桥接热区保证
  慢速移动不断链；键盘通路补齐（触发行 tabbable + focus 揭示，鼠标点击不夺焦）。
- **内联滑条**（Codex 式）：点「记忆范围」原地展开（`grid-rows` 生长动画），三停点
  日常/工作/智能，拖拽跨档芯片文字**实时**联动，键盘方向键/Home/End + `aria-valuetext`；
  折叠态 `visibility: hidden` 移出可访问树。
- **暂停恢复快照**：数据流切「暂停」进 off 档时 host 持久化暂停前范围与注入覆盖，
  恢复即原样还原（`session-modes.json` 无损兼容旧数据）。
- **占用只住官方上下文环面板**：0.8.8 的外圈记忆光晕弧移除（官方环恢复逐比特原生），
  官方面板底部「记忆」分项小节（召回片段/记忆稳定区）成为占用唯一展示处；composer
  区新增 `待蒸馏 N` 统计行遥测段（官方 stats 行语法，自适应轮询）。

#### 设置侧（记忆工作台五区）

- **总览**：健康摘要（存储/向量检索/蒸馏队列子系统 chip + 注意区）+ 最近活动前 5 条 +
  关键数字（记忆资产/场景/本周蒸馏输出/上次蒸馏）+ 引导式空状态。
  ![工作台总览](./assets/changelog/0.9.0/02-工作台总览.png)
- **记忆库**：L1 记忆 / L2 场景 / L3 画像按更新时间混排的**只读资产活动流**——搜索
  （L1 走 FTS 索引）+ 类型/族/时间筛选 + 行内原位展开（完整内容 + 元信息 + 复制）+
  游标分页。
  ![记忆库活动流](./assets/changelog/0.9.0/03-记忆库.png)
- **自动化**：基础四开关（总闸/捕获/蒸馏/召回）+ 语义检索与蒸馏路由摘要行 + 高级披露
  （路由链编辑器/预算）+ 嵌入模型披露（既有编辑器原样迁入）。
- **洞察**：成本（原成本看板）/ 活动（近 7 天资产活动条形图 + 蒸馏调用失败表）/
  召回（全量累计 + 停用分布）三子页。
  ![洞察·活动](./assets/changelog/0.9.0/04-洞察活动.png)
- **维护**：运行健康行 + 数据目录/版本 + 诊断日志披露 + 危险区重建（原生 Modal 确认）。

#### Host 侧（三个只读聚合端点）

- `workspace-overview`（总览单发聚合）、`asset-activity`（活动流：L1 索引查询/检索缝、
  L2 场景页内补读、L3 画像 mtime，混排 + base64url 游标 + 筛选）、`runtime-insights`
  （逐日活动 SQL GROUP BY + llm-usage 计数器 + 召回注册表聚合 + 档位停用分布）。
  全部只读、索引化，不改蒸馏/召回语义；`listL1` 增加 since 下限（走 idx_l1_updated）。

### 验证

- 宿主 0.1.2-alpha.2 真机全链路：芯片级联菜单/滑条键盘切档（芯片文字实时联动）/hover
  子面板、工作台五区全部真实数据渲染（活动流 30 条混排、停用分布持久化计数）；
  typecheck×2 / build / smoke（33 节，§21 断言随组件删除同步重写）全绿。
