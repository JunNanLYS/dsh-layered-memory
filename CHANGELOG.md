...
- **仅画像（persona-only）会话模式**：数据流新增第五档「仅画像」——介于读写与只写
  之间的读侧收窄：捕获/蒸馏照常，跳过 L1 动态召回注入，但 L3 画像与场景导航稳定区
  照常注入、`memory_search` 等读工具仍可用。适合上下文连贯的会话（终端 CLI、长任务
  工作台）——语境自足时避免无关记忆的自动干扰，同时保留画像等稳定上下文与按需查询。
  实现上把 #38 的会话级注入覆盖从布尔扩展为三态（`true` / `'persona'` / `false`，
  缺省跟随全局），旧布尔值自动兼容迁移；读侧三闸门（pre-step 消息注入 / 稳定区 /
  工具）按新语义差异化放行。覆盖按会话持久化，芯片/级联菜单/工作台同步显示。

### 验证

- typecheck ×2 / build / smoke（新增 persona 覆盖往返、注入闸门差异、稳定区保留、
  只写分界断言）/ verify-catalog 全绿。
