...
export class MemoryDb {
      ...
      try {
        ...
        if (this.ftsAvailable) {
          try {
            ...
            this.logger?.warn(
              `${TAG} L1 FTS 写入失败（非致命）id=${record.id}: ${ftsErr instanceof Error ? ftsErr.message : String(ftsErr)}`,
            );
          }
        }
        this.db.exec('COMMIT');
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        if (this.ftsAvailable) {
          try {
            ...
            this.logger?.warn(
              `${TAG} L0 FTS 写入失败（非致命）id=${r.id}: ${ftsErr instanceof Error ? ftsErr.message : String(ftsErr)}`,
            );
          }
        }
      }
  ...
  getL1ForReindex(): Array<{ id: string; content: string }> {
    if (this.degraded) return [];
    return this.db
  ...
  getL0ForReindex(): Array<{ id: string; text: string }> {
    if (this.degraded) return [];
    return this.db
...
function isZeroVector(vec: Float32Array): boolean {
  for (const v of vec) {
    if (v !== 0) return false;
  }
  return true;
