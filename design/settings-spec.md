...
- 行结构（两行）：徽标（主/序号，定宽 20）+ 供应商 NSel + 模型 NSel（或降级
  NInput，供应商无目录时回车提交）为第一行；档位 NSel（宽 150，「跟随部署配置」+
  该模型 efforts，未选定模型时禁用）+ ↑ ↓ ✕ 为第二行，且经 28px 前导占位
  （徽标 20 + 间距 8）与供应商下拉**左对齐**（NButton 26×26 图标钮，inline
