...
> 已落地实现的**事实记录**（0.8.8 起面板分项；0.9.0 移除外圈光晕弧，
> 占用只住官方上下文环面板，composer 区零占用 UI）。
> 冲突时以代码为准并回写本文件。
...
2 单位 / accent 光晕）。0.9.0 重构整体移除：官方环恢复逐比特原生，
...
- 内容自上而下：标题「记忆占用」（text-2 加粗）→
  ...
  不显示合计占比行与口径声明——占比语义由图形承担，口径记录在本文档而非 UI。
...
1. 文案硬编码中文；无插件级 locale 钩子前不承诺多语言。
